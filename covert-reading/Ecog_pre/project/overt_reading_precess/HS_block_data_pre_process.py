...
class HSblock_pre_process:
    ...
    def __get_true_onset(self, block, cue=False):
        ...
        print(textname)
        if os.path.exists(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}oneset_cue_point.npy'):
            os.remove(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}oneset_cue_point.npy')
        with open(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}oneset_cue_point.npy','wb')as file:
            np.save(file,onset_cue_time)

        if os.path.exists(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}onset_read_time.npy'):
            os.remove(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}onset_read_time.npy')
        with open(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}onset_read_time.npy','wb')as file:
            np.save(file,onset_read_time)
    ...
    def __get_delay(self, eprime, ecog, block):
        ...
        res1=res.diag()[self.syn_point[f'HS{self.HS}'][str(block)]].numpy()
        delay=np.mean(res1)
        print(delay)
        if os.path.exists(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}delay.npy'):
            os.remove(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}delay.npy')
        with open(f'/public/DATA/overt_reading/points/HS{self.HS}/{block}delay.npy','wb')as f:
            np.save(f,delay)
